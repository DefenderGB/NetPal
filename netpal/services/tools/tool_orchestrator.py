...
services, handling tool matching, httpx dependency for HTTP tools,
...
class ToolOrchestrator:
    ...
    1. Run httpx first for web services (provides response data)
    ...
    def __init__(self, project_id: str, config: dict):
        ...
        self.httpx = HttpxRunner(project_id, config)
    ...
    def execute_tools_for_service(
        ...
        Runs httpx first for web services, then matches and executes
        ...
        # Run httpx first for web services
        httpx_result_file = None
        httpx_screenshot_file = None
        httpx_response_file = None
        
        if self.httpx.can_run_on_service(service):
            if self._should_skip_tool('auto_httpx', rerun_autotools, existing_proofs):
                if callback:
                    callback(f"\n[SKIP] HTTPX already ran on {host.ip}:{service.port} (rerun_autotools={rerun_autotools})\n")
            else:
                if callback:
                    callback(f"\n[AUTO] Running HTTPX on {host.ip}:{service.port}\n")
                
                httpx_result = self.httpx.execute(host, service, asset_identifier, callback)
                
                if httpx_result.error and callback:
                    callback(f"[WARNING] HTTPX encountered an error: {httpx_result.error}\n")
                    callback(f"[WARNING] HTTP-based tools may not function properly for this service\n")
                
                httpx_result_file = self.httpx.get_result_file(httpx_result)
                httpx_screenshot_file = self.httpx.get_screenshot_file(httpx_result)
                httpx_response_file = self.httpx.get_response_file(httpx_result)
                
                if (httpx_result_file or httpx_screenshot_file) and not httpx_result.error:
                    results.append(('auto_httpx', httpx_result_file, httpx_screenshot_file, [], httpx_response_file))
        ...
        for tool in matching_tools:
            ...
            proof_type = self._proof_type_for_tool(tool_type, safe_tool_name)
    ...
    @staticmethod
    def _proof_type_for_tool(tool_type: str, safe_tool_name: str) -> str:
    ...
    def _should_skip_tool(
        ...
        Args:
            proof_type: The proof type key (e.g. ``"auto_httpx"``).
    ...
    def _execute_configured_tool(
        ...
        service: Service,
        asset_identifier: str,
        httpx_result_file: Optional[str],
        callback=None
        ...
        Args:
            ...
            httpx_result_file: Path to httpx response file (for http_custom)
            callback: Optional output callback
    ...
    def _run_nmap_custom(
        ...
        if result.output_files:
            ...
            return (f"nmap_{safe_tool_name}", output_file, None, [], None)
    ...
    def _run_http_custom(
        ...
        if not httpx_result_file:
            if callback:
                callback(f"[SKIPPED] {tool.get('tool_name', 'Unknown')}: requires httpx response (web service detection may have failed)\n")
            return None
        
        result = self.http_custom.execute(
            host, service, asset_identifier, callback,
            tool_config=tool, httpx_response_file=httpx_result_file
        )
        ...
        if self.http_custom.did_match(result):
            ...
            return (f"http_{safe_tool_name}", output_file, None, [], None)
        elif callback:
    ...
    def _run_nuclei(
        ...
        if result.output_files or result.findings:
            output_file = result.output_files[0] if result.output_files else None
            return (f"nuclei_{safe_tool_name}", output_file, None, result.findings, None)
