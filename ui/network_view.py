...
def build_nmap_command_preview(target: str, scan_type: str, custom_ports: str = None,
                                interface: str = None, use_file: bool = False) -> str:
    """
    ...
    # Add scan type specific flags (matching scan_types.yaml)
    if scan_type == "ping":
        cmd_parts.extend(["-sn", "-v"])
    elif scan_type == "top1000":
        cmd_parts.extend(["--top-ports", "1000", "-sV", "-v"])
    elif scan_type == "all_ports":
        cmd_parts.extend(["-p-", "-sV", "-v"])
...
def _build_command_preview_data(target_type, target_data, scan_type, project_name, network_range=None):
    ...
    if target_type == TARGET_TYPE_LIST_ENDPOINTS and isinstance(target_data, Network):
        if target_data.host_list_path:
        ...
        if discovered_ips_file_exists(project_name, network_range) and scan_type != "custom":
            target_preview = discovered_ips_path
            use_file = True
        else:
            active_ips = [host.ip for host in target_data.hosts]
            target_preview = " ".join(active_ips)
...
def _build_target_options_for_network(network, project_name):
    ...
    if network.asset_type == "list":
        ...
        if network.hosts:
            ...
            # Option 3+: Individual active hosts
            for host in network.hosts:
                host_label = f"{host.ip}" + (f" ({host.hostname})" if host.hostname else "")
                target_options.append(host_label)
                target_map[host_label] = (TARGET_TYPE_SINGLE_HOST, host.ip)
...
def _scan_single_host(scanner, target_ip, scan_type, custom_ports, project_name,
                     output_callback, network_interface):
...
def _scan_list_endpoints(scanner, target_network, scan_type, custom_ports,
    ...
    if target_network.host_list_path:
        ...
        if scan_type == "custom" and custom_ports:
            # For custom ports, still read endpoints and use scan_ports
...
def _scan_active_hosts(scanner, target_data, scan_type, custom_ports,
    ...
    if discovered_ips_file_exists(project_name, network.range):
        ...
        if scan_type == "custom" and custom_ports:
            # For custom ports, still read IPs and use scan_ports
            active_ips = [host.ip for host in target_data.hosts]
            return scanner.scan_ports(
...
def _process_scan_results(hosts, error, output_text, output_container, scan_type,
                         project, network, st_session_state):
    """
    ...
    if error:
        ...
        # Add auto-tool status to output with details
        ...
        # NOW store the complete output with all post-scan messages
        full_output = ''.join(output_text)
        st_session_state.last_scan_output = full_output
...
def render_network_details_dialog(dm: DialogManager):
    ...
    project = st.session_state.current_project
    network_idx = st.session_state.selected_network_idx
    
    if network_idx is None or network_idx >= len(project.networks):
        st.error("Invalid asset selected")
        return
    
    network = project.networks[network_idx]
    ...
    if network.asset_type == "list":
        with st.expander("📋 Endpoints in List", expanded=True):
...
def render_networks_page():
    ...
    if 'selected_network_idx' not in st.session_state:
        st.session_state.selected_network_idx = None
    ...
    if not project.networks:
        ...
        network_data = []
        for idx, network in enumerate(project.networks):
            # Count totals
            total_hosts = len(network.hosts)
            total_services = sum(len(host.services) for host in network.hosts)
            total_findings = sum(len(host.findings) for host in network.hosts)
            ...
            if network.asset_type == "list":
                ...
                endpoints_count = len(network.endpoints) if network.endpoints else 0
            else:
            ...
            # Add endpoints count for list types
            if endpoints_count is not None:
                row_data["Endpoints"] = endpoints_count
        ...
        if hasattr(event, 'selection') and hasattr(event.selection, 'rows') and event.selection.rows:
            ...
            # Store selection in session state for persistence across reruns
            st.session_state.selected_network_idx = selected_idx
            selected_network = project.networks[selected_idx]
        elif st.session_state.selected_network_idx is not None and st.session_state.selected_network_idx < len(project.networks):
            # Restore previous selection from session state (maintains selection after scan completion)
            selected_idx = st.session_state.selected_network_idx
            selected_network = project.networks[selected_idx]
        else:
            # No selection
            selected_idx = None
        ...
        if selected_network:
            ...
            with col2:
                ...
                if st.button("🗑️ Delete Asset", key="delete_network", type="secondary", width='stretch', disabled=scan_active):
                    ...
                    project.networks.pop(selected_idx)
                    st.session_state.storage.save_project(project)
                    st.session_state.selected_network_idx = None  # Clear selection after deletion
...
def _render_scan_interface_core(
    ...
    if scan_active:
        ...
        st.code(st.session_state.last_scan_output, language="bash", height=UI_HEIGHT_LARGE, wrap_lines=True)
