...
def render_host_view():
    ...
    # Action buttons row
    col_title, col_merge = st.columns([3, 1])
    with col_title:
        st.subheader(f"Hosts ({len(all_hosts)} total)")
    with col_merge:
        if st.button("🔀 Merge Duplicate Hosts", key="merge_duplicates", help="Find and merge hosts with duplicate IPs across all networks"):
            if check_and_reload_if_stale():
                project = st.session_state.current_project
            
            # Use the new cross-network merge method
            duplicates_found, merged_ips = project.merge_duplicate_hosts_across_networks()
            
            if duplicates_found > 0:
                st.session_state.storage.save_project(project)
                
                # Display detailed merge information
                st.success(f"✅ Merged {duplicates_found} duplicate host(s) across networks")
                
                if merged_ips:
                    st.info(f"**Merged IPs:** {', '.join(merged_ips)}")
    ...
    for network_range, host in filtered_hosts:
        ...
        # Add star prefix for interesting hosts
        ip_display = f"⭐ {host.ip}" if getattr(host, 'is_interesting', False) else host.ip
...
def render_host_details(project, network, host):
    ...
    if st.button(button_text, key="toggle_interesting", width='stretch'):
        ...
        st.session_state.storage.save_project(project)
        st.success(f"Host {host.ip} {'unmarked' if is_interesting else 'marked'} as interesting")
        st.rerun()
                ...
                if 'screenshot' in proof['type'].lower() or content.endswith(('.png', '.jpg', '.jpeg')):
                    ...
                    try:
                        with open(content, 'r') as f:
                            file_content = f.read()
                        st.code(file_content, language="text", height=450)
                    except Exception as e:
