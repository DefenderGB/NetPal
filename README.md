...
**AI Chatbot**:
- Accessible in Online or Offline mode, need to set credentials in config/ai_settings.ymal
...
- Choose scan type:
  ...
  - `all_ports`: Scans all ports with service detection
...
- `{protocol}`: To append http or https
- `{username}`: To append username from local credential storage
- `{password}`: To append password from local credential storage
