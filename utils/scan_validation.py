...
def validate_scan_configuration(
    ...
    Dynamically loads valid scan types from config/scan_types.yaml to ensure
    validation stays in sync with available scan types.
    
    Args:
        target: Target network/IP (if direct target)
        scan_type: Type of scan (ping, top1000, custom, all_ports, top100, etc.)
    ...
    """
    # Validate scan type by dynamically loading from YAML
    if scan_type:
        # Load all scan types from configuration
        scan_types_config = ConfigLoader.load_scan_types()
        valid_scan_types = [st.get('id') for st in scan_types_config if st.get('id')]
        
        if scan_type not in valid_scan_types:
            return False, f"Invalid scan type: {scan_type}. Must be one of: {', '.join(sorted(valid_scan_types))}"
...
def validate_target_selection(
    ...
    Returns:
        Tuple of (is_valid, error_message)
    """
    valid_target_types = ["network", "active_hosts", "single_host", "list_endpoints", "chunk_file", "disabled"]
    
    if target_type not in valid_target_types:
        return False, f"Invalid target type: {target_type}"
    
    # Disabled targets should not be validated for scanning
    if target_type == "disabled":
        return False, "Cannot scan a disabled target (section header)"
